...
import { mockAgencyMembers, mockClients } from '@/lib/mock-data';
...
const getClientAvatarColors = (name: string) => {
  // Create a simple hash of the name to get a consistent palette index
...
export default function MessagesPage() {
  ...
  useEffect(() => {
    // Create contacts from team members
    const teamContacts: Contact[] = mockAgencyMembers.map(member => ({
      id: member.id,
      name: member.name,
      email: member.email,
    ...
    const clientContacts: Contact[] = mockClients.map(client => ({
      ...
      name: client.name,
      email: client.email,
      avatar: '', // We'll render Boring Avatars directly in the component
      type: 'client',
    ...
    const allContacts = [...teamContacts, ...clientContacts];
    ...
    const sampleConversations: Conversation[] = allContacts.map((contact, index) => ({
      ...
      // avatarColors: contact.avatarColors, // Removed this line as it's undefined for team members
      lastMessage: contact.type === 'team' 
        ? "Latest project update looks great!" 
      ...
      messages: [
        {
          id: `msg-${index}-1`,
          content: contact.type === 'team' 
            ? `Hi, let's discuss the latest updates for ${contact.role === 'designer' ? 'the design' : 'development'}.` 
            : `Hello, I wanted to check in about your project with ${contact.company}.`,
        ...
        },
        {
          id: `msg-${index}-2`,
          content: contact.type === 'team' 
            ? "Sure thing! I have some progress to share." 
            : "Thanks for checking in. We're really happy with the progress.",
  ...
  const displayedConversations = filteredConversations.filter(conversation => {
    ...
    if (activeTab === "team") return conversation.contactType === 'team';
    if (activeTab === "clients") return conversation.contactType === 'client';
    return true;
  });
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-start">
                        <div className="font-medium truncate flex items-center gap-1.5">
                          ...
                          {conversation.contactType === 'team' ? (
                            <Badge variant="outline" className="text-xs bg-blue-50 text-blue-700 border-blue-200 font-normal py-0 h-5">Team</Badge>
                          ) : (
                      ...
                      <div className="text-xs text-gray-400">
                        {conversation.contactType === 'team' 
                          ? `Team • ${mockAgencyMembers.find(m => m.id === conversation.contactId)?.role || 'Member'}`
          ...
          <div className="flex-1 flex flex-col bg-white">
            ...
            <div className="border-b p-4 flex justify-between items-center">
              <div className="flex items-center gap-3">
                {activeConversation.contactType === 'team' ? (
                  <Avatar className="ring-2 ring-blue-200">
                    <AvatarImage src={activeConversation.avatarSrc} />
                    <AvatarFallback>
                ...
                <div>
                  <div className="font-medium flex items-center gap-2">
                    ...
                    {activeConversation.contactType === 'team' ? (
                      <Badge variant="outline" className="text-xs bg-blue-50 text-blue-700 border-blue-200 font-normal">Team</Badge>
                    ) : (
                  ...
                  <div className="text-xs text-gray-500">
                    {activeConversation.contactType === 'team' 
                      ? `${mockAgencyMembers.find(m => m.id === activeConversation.contactId)?.role || 'Team Member'}`
