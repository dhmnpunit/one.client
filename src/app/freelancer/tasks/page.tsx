...
interface Task {
  ...
  dueDate: string;
  assignedToId: string; // In freelancer context, this might always be the freelancer's ID
  createdBy: string;
  createdAt: string;
  updatedAt: string;
  completedAt?: string;
...
// Mock data (Adjustments for freelancer context)
...
// For freelancer, teamMembers might just include the freelancer themselves
const teamMembers: TeamMember[] = [
  { id: "freelancer-1", name: "Sercan Yayla", email: "sercan.yayla@example.com", role: "freelancer", avatar: "/avatars/sercan-yayla.jpg" }
];

const freelancerId = "freelancer-1"; // Assume this is the logged-in freelancer's ID

// Mock tasks data (Adjust assignedToId and createdBy for freelancer context)
const tasks: Task[] = [
  {
    id: "task-1",
    ...
    projectId: "1", // Updated projectId to match mock projects
    status: "in-progress",
    priority: "high",
    dueDate: "2024-08-15",
    assignedToId: freelancerId,
    createdBy: freelancerId, // Or a client ID if applicable
    createdAt: "2024-07-20T10:00:00Z",
    updatedAt: "2024-07-28T14:30:00Z",
    progress: 60,
    estimatedHours: 8,
    actualHours: 10,
    activities: [ { timestamp: "2024-07-28T14:30:00Z", text: "updated progress to 60%", userId: freelancerId }, { timestamp: "2024-07-20T10:00:00Z", text: "created the task", userId: freelancerId } ],
    comments: [ { id: "c1", text: "Ensure the primary CTA stands out.", userId: "client-1", timestamp: "2024-07-25T09:30:00Z" }, { id: "c2", text: "Agreed. I'll use a contrasting color and larger size.", userId: freelancerId, timestamp: "2024-07-25T10:15:00Z" } ],
  ...
  },
  {
    ...
    {
    id: "task-8",
    title: "Finalize Report Layout",
    description: "Apply final design touches to the annual report",
    projectId: "6", // Assuming project 6 exists or adjust
    status: "completed",
    priority: "medium",
    dueDate: "2024-01-15",
    assignedToId: freelancerId,
...
// Renamed function to FreelancerTasksPage
export default function FreelancerTasksPage() {
  // Consolidated filter state
  const [filters, setFilters] = useState({
    project: 'all',
    statuses: new Set<string>(),
    // Assignee filter might be less relevant or fixed for freelancer
    assignee: freelancerId, // Default/fixed to the freelancer 
    priority: 'all',
  });
  // Removed 'my-tasks' tab as all tasks are the freelancer's
  ...
  // Helper to check if any filters are active (excluding 'all' and fixed assignee)
  const areFiltersActive = useMemo(() => {
    return filters.project !== 'all' ||
           filters.statuses.size > 0 ||
           // filters.assignee !== freelancerId || // Assignee filter might be removed
           filters.priority !== 'all';
  }, [filters]);

  // Get team member (freelancer) by ID
  const getTeamMember = (id: string | undefined): TeamMember => {
     // Simplified for freelancer: always return the freelancer's info
     const member = teamMembers.find(m => m.id === id);
     return member || { id: '', name: 'Unknown', email: '', role: '', avatar: '' };
  };
  ...
  // --- Calculate Stats (using the full 'tasks' list, assuming all belong to freelancer) ---
  const today = useMemo(() => { const d = new Date(); d.setHours(0, 0, 0, 0); return d; }, []);
  const oneWeekFromNow = useMemo(() => { const d = new Date(today); d.setDate(d.getDate() + 7); return d; }, [today]);

  // Simplified Stats Calculation for Freelancer
  const openStats = useMemo(() => tasks.filter(t => t.status !== 'completed').length, [tasks]);
  const overdueStats = useMemo(() => tasks.filter(t => isOverdue(t)).length, [tasks, isOverdue]);
  const dueThisWeekStats = useMemo(() => tasks.filter(t => {
    if (t.status === 'completed') return false;
    const dueDate = new Date(t.dueDate); dueDate.setHours(0, 0, 0, 0);
    return dueDate >= today && dueDate < oneWeekFromNow;
  }).length, [tasks, today, oneWeekFromNow]);
  const inReviewStats = useMemo(() => tasks.filter(t => t.status === 'review').length, [tasks]);

  // Update Filtering Logic for Freelancer
  const filteredTasks = useMemo(() => {
    return tasks.filter(task => {
      ...
      // Assignee filter is implicitly freelancerId if not removed entirely
      const matchesPriority = filters.priority === 'all' || task.priority === filters.priority;

      return matchesProject && matchesStatus && matchesPriority;
    });
  }, [filters, activeTab, isOverdue]);
  ...
  const clearFilters = () => {
    setFilters({
      project: 'all',
      statuses: new Set<string>(),
      assignee: freelancerId, // Reset to freelancer
      priority: 'all',
    });
  };
  ...
  const handleStatCardClick = (type: 'open' | 'overdue' | 'dueThisWeek' | 'inReview') => {
    switch (type) {
      case 'overdue':
        ...
        clearFilters(); // Clear other filters when clicking overdue
        break;
      case 'inReview':
        setActiveTab('all'); // Stay on 'all' but filter by status
        setFilters({
          project: 'all',
          statuses: new Set<string>(['review']),
          assignee: freelancerId,
          priority: 'all',
        });
        break;
      case 'dueThisWeek':
         setActiveTab('all'); // Stay on 'all', filtering happens implicitly in calculation
         alert("Filtering by 'Due This Week' not implemented yet via cards."); // Placeholder
        break;
      case 'open':
         setActiveTab('all');
         setFilters({
           project: 'all',
           statuses: new Set<string>(['todo', 'in-progress', 'review']), // Explicitly set open statuses
           assignee: freelancerId,
           priority: 'all',
         });
         break;
  ...
  const handleCreateTask = useCallback(() => {
    ...
    const newTask: Task = {
      ...
      assignedToId: freelancerId, // Fixed assignee
      createdBy: freelancerId, // Assume freelancer creates tasks
    ...
    newTaskDescription, newTaskEstHours
  ]);
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {/* Simplified Task Stats Cards for Freelancer */}
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
        ...
        >
          ...
          <div className="text-2xl font-bold">{openStats}</div>
          {/* Removed "Mine:" subtext */}
        </button>
        <button 
          className="bg-white rounded-lg border p-4 text-left hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2" 
          ...
          <div className="text-2xl font-bold">{overdueStats}</div>
          {/* Removed "Mine:" subtext */}
        </button>
        <button 
          className="bg-white rounded-lg border p-4 text-left hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2" 
          ...
          <div className="text-2xl font-bold">{dueThisWeekStats}</div>
          {/* Removed "Mine:" subtext */}
        </button>
        <button 
          className="bg-white rounded-lg border p-4 text-left hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2" 
          ...
          <div className="text-2xl font-bold">{inReviewStats}</div>
          {/* Removed "Mine:" subtext */}
        </button>
      </div>

      {/* Tabs and Filter Bar Row (Simplified for Freelancer) */}
      <div className="flex justify-between items-center">
        {/* Left: Tabs (Simplified) */} 
        <Tabs defaultValue="all" value={activeTab} onValueChange={(value) => setActiveTab(value as 'all' | 'overdue')} variant="folder" className="w-auto">
          <TabsList>
            <TabsTrigger value="all" className="flex-1 flex items-center gap-2">
              <ListChecks className="h-4 w-4" />
              All Tasks
            </TabsTrigger>
            {/* Removed My Tasks Tab */}
        ...
        <div className="flex items-center gap-2">
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-56">
              ...
              {/* Assignee Filter - Removed for Freelancer */}
              {/* 
              <DropdownMenuSub>
              ...
              <div className="rounded-md border overflow-hidden">
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-[35%]">Task</th>
                      ...
                      {/* Removed Assignee Column for freelancer */}
          ...
          <div className="h-full flex flex-col">
            ...
            <div className="p-4 border-b border-gray-200 bg-white space-y-3">
              ...
              <div className="flex flex-wrap gap-x-4 gap-y-2 text-sm text-gray-600">
                ...
                {/* Removed Assignee from header for freelancer */}
                {/* <div className="flex items-center space-x-1.5">
                ...
                <TabsContent value="details" className="space-y-6 mt-0">
                  ...
                  <div className="bg-white rounded-lg p-4 border border-gray-200">
                    ...
                    <div className="divide-y divide-gray-100">
                      ...
                      {/* Client */} 
                      <div className="py-2 flex items-center">
                        <span className="w-1/3 text-sm text-gray-500 flex items-center"><User className="h-4 w-4 mr-2 opacity-60"/>Client</span>
                        <span className="w-2/3 text-sm font-medium text-gray-800 hover:text-blue-600 cursor-pointer">
                          {getProject(selectedTask.projectId).client} 
                        </span>
                      </div>
                       {/* Created By - Simplified for freelancer */}
                      <div className="py-2 flex items-center">
                    ...
                    <div className="p-4 border-t border-gray-200 bg-gray-50">
                       <div className="flex items-start space-x-3">
                         <Avatar className="h-8 w-8 mt-1"> 
                            <AvatarImage src={`https://api.dicebear.com/8.x/micah/svg?seed=${encodeURIComponent(getTeamMember(freelancerId).name)}`} alt={getTeamMember(freelancerId).name} />
                            <AvatarFallback className="text-xs">{getTeamMember(freelancerId).name.charAt(0)}</AvatarFallback>
                         </Avatar>
