...
// Define Freelancer Info
const freelancerId = "freelancer-1"; // Assuming this is the logged-in freelancer's ID
const freelancerName = "Sercan Yayla"; // Freelancer's name
...
interface Client {
  ...
  name: string;
  company: string;
  avatarColors: string[]; // Store colors instead of pre-generated URL
}

// --- NEW: Define color palettes consistent with clients page ---
...
// --- MODIFIED: Mock Client Data with avatarColors ---
const clients: Client[] = [
  { id: "client-acme", name: "Alice Chen", company: "Acme Corp", avatarColors: avatarColorPalettes[0] },
  { id: "client-tech", name: "Bob Williams", company: "TechStart", avatarColors: avatarColorPalettes[1] },
  { id: "client-green", name: "Charlie Davis", company: "GreenLife", avatarColors: avatarColorPalettes[2] },
  { id: "client-innovate", name: "Diana Rodriguez", company: "InnovateTech", avatarColors: avatarColorPalettes[3] },
  { id: "client-fashion", name: "Ethan Kumar", company: "FashionForward", avatarColors: avatarColorPalettes[4] },
  { id: "client-global", name: "Fiona Miller", company: "Global Finance", avatarColors: avatarColorPalettes[0] } // Re-use palette for example
];

// Mock documents data (Updated uploadedBy to reflect freelancer and clients)
const documents: Document[] = [
  // Website Redesign Documents
  {
    ...
    type: "pdf",
    size: "4.2 MB",
    uploadedBy: freelancerId, // Uploaded by Freelancer
  ...
  },
  {
    ...
    type: "pdf",
    size: "2.8 MB",
    uploadedBy: "client-acme", // Uploaded by Client
  ...
  },
  {
    ...
    type: "doc",
    size: "1.2 MB",
    uploadedBy: "client-tech", // Uploaded by Client
  ...
  },
  {
    ...
    type: "pdf",
    size: "7.8 MB",
    uploadedBy: "client-green", // Uploaded by Client
  ...
  // Marketing Campaign Documents
  {
    ...
    type: "doc",
    size: "4.5 MB",
    uploadedBy: "client-innovate", // Uploaded by Client
  ...
  },
  {
    ...
    type: "spreadsheet",
    size: "2.3 MB",
    uploadedBy: "client-fashion", // Uploaded by Client
  ...
  },
  {
    ...
    type: "spreadsheet",
    size: "1.8 MB",
    uploadedBy: "client-global", // Uploaded by Client
...
// --- NEW: Generate avatar URL for the Freelancer using DiceBear Micah ---
function getFreelancerAvatar(name: string) {
  const seed = name.toLowerCase().replace(/\s+/g, '-');
  // Using v8 as v7 might not be available or stable
...
export default function DocumentsPage() {
  ...
  // --- MODIFIED: Get document uploader name and data ---
  const getUploaderData = (id: string): { name: string; avatarSrc: string | string[]; fallback: string; isFreelancer: boolean } => {
    if (id === freelancerId) {
      return {
        name: "You",
        avatarSrc: getFreelancerAvatar(freelancerName), // DiceBear URL for Freelancer
        fallback: freelancerName.charAt(0).toUpperCase(),
        isFreelancer: true
      };
    }
    ...
    if (client) {
      return {
        name: client.name,
        avatarSrc: client.avatarColors, // Color array for Client
        fallback: client.name.charAt(0).toUpperCase(),
        isFreelancer: false
      };
    }
    return { name: 'Unknown', avatarSrc: [], fallback: '?', isFreelancer: false }; // Default fallback
  };
                ...
                <table className="min-w-full divide-y divide-gray-200 table-fixed">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {docs.map(doc => {
                      ...
                      // --- NEW: Get uploader info ---
                      const uploader = getUploaderData(doc.uploadedBy);
                        ...
                        <tr key={doc.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-6 py-2 whitespace-nowrap">
                            <div className="flex items-center">
                              ...
                              {uploader.isFreelancer ? (
                                <Avatar className="h-8 w-8"> 
                                  <AvatarImage src={uploader.avatarSrc as string} alt={uploader.name} />
                                  <AvatarFallback>{uploader.fallback}</AvatarFallback>
                                </Avatar>
                              ) : (
                                <BoringAvatar
                                  size={32} // h-8 w-8 corresponds roughly to 32px
                                  name={uploader.name} // Use name for uniqueness
                                  variant="beam"
                                  colors={uploader.avatarSrc as string[]}
                                />
                              )}
                              {/* --- MODIFIED: Display "You" or client name --- */}
                              <span className="ml-2 text-sm text-gray-900">{uploader.name}</span>
                            </div>
                          </td>
